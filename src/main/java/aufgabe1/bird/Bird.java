...
public class Bird {
  ...
  /**
   * In produktivem Code wäre diese Methode `private`, und man würde im Test statt
   * `setEnergy` die `eat` Methode verwenden
   * (da diese bereits separat getestet wurde). Um jedoch zu vermeiden, dass wenn
   * jemand `eat` nicht implementiert hat
   * nicht Abzug bei der Implementierung von `fly` bekommt, wird diese Methode in
   * den Tests verwendet.
   */
  ...
  public boolean addPreferredFood(Food food) {
    return preferredFoods.add(food);
  }
  ...
  public boolean eat(Food food) {
    ...
    for (Food preferedFood : preferredFoods) {
      if (food.equals(preferedFood)) {
        eaten.add(food);
        energy += food.getEnergyContent();
        return true;
      }
